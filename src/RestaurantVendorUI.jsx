...
const RestaurantVendorUI = () => {
  ...
  const placeOrder = () => {
    ...
    setOrders([
      ...orders,
      { id: Date.now(), token: selectedToken, items: cart, total },
    ]);
    setCart([]);
    setCartOpen(false);
  };
          ...
          <div key={category} className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="bg-white border rounded-xl p-3 flex items-center gap-3 shadow-sm hover:shadow-md transition"
                >
                  {/* Small image */}
                  ...
                  <div className="flex flex-col flex-1 items-end gap-1">
                    <div className="text-sm font-medium text-right leading-snug">
                      {item.name}
                    </div>
        ...
        {orders.length > 0 && (
          <div className="mt-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {orders.map((o) => (
                <div key={o.id} className="border bg-white p-3 rounded-lg text-sm">
